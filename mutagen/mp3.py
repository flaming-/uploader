...
class MPEGInfo(object):
    ...
    flaming added:
    CBR -- True if it is CBR, False if ABR/VBR, possibly 'unknown' (see code)
    encoder -- LAME, FHG, etc.
    encoder_version -- x.xxx (3.99b, etc)
    lowpass -- the lowpass value of the encoder settings
    min_bitrate -- minimum bitrate, maximum value of 255 (see code for notes)
    VBR_quality -- 0 to 100. if not VBR: None
    VBR_preset -- LAME VBR preset: V2, V0, etc. if not lame VBR: None

    data -- raw, for debugging. disabled by default (because it's very long)
    """
    ...
    def __try(self, fileobj, offset, real_size, check_second=True):
        ...
        data_saved = data  ## for later use --flaming
        ...
        #note: 'Xing' in header only exists if the file is VBR / ABR.
        #CBR files instead have 'Info' (and btw it's slot 36)
        magic = data [36:40]
        if magic == 'Xing':
            self.CBR = False
        elif magic == 'Info':
        ...
        # Finding the encoder and LAME VBR setting (if it exists)
        self.encoder  = data_saved[156:160]
        self.encoder_version = data_saved[160:165]

        self.lowpass = ord(data_saved[166]) * 100   #see mp3 info tag docs
        self.min_bitrate = ord(data_saved[176])
            #if VBR/CBR: corresponds to the -b flag ("-b 320" for example)
            #if ABR: is the average bitrate (min_bitrate is a misnomer in that
            #situation.
            #note that the maximum value for this field is 255. If the actual
            #value is higher (e.g. a CBR 320 file) it still maxes out at 255.
        self.data = data_saved              #use this to debug / explore
        
        if self.encoder == 'LAME' and not self.CBR:
            self.VBR_quality = ord(data_saved[155])
            V = max((100-self.VBR_quality),0)//10
            self.VBR_preset = 'V%s' % V
        else:
